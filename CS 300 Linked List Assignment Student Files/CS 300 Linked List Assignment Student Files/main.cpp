...
void LinkedList::Append(Bid bid) {
    ...
    //increase size count by one or the link list won't be accurate
    size ++;
}
...
void LinkedList::PrintList() {
    ...
    // start current node point at the head
    Node *current = head;

    // while loop over each node looking for a match  
    while(current){
        //output print the current bidID, title, amount and fund
        cout << current -> bid.bidId <<" : "<<current ->bid.title << " | "<< current->bid.amount << " | " << current->bid.fund <<endl;
      
      //set current equal to next otherwise it will never finish and end up with infinite loop
...
void LinkedList::Remove(string bidId) {
    ...
    Node *current = head;
    //if current and current point to bidID equal to the bidID
    ...
    while (current->next) {
        ...
        if (bidId == current->next->bid.bidId){
          ...
          // current node is equal to next node otherwise loop won't finish and get stuck
        current = current ->next;
    }
    return;
    }
...
Bid LinkedList::Search(string bidId) {
    ...
    //while current node
    while (current) {     
        //if the bid id equal to current node point it to the bid id
